...
if (isset($_POST['save'])) { // if save button on the form is clicked
    ...
    $sql_e = "SELECT * FROM rekini WHERE name='$filename'";
  	$res_e = mysqli_query($conn, $sql_e);
        
    
    // destination of the file on the server
    $destination = 'rekini/' . $filename;
    ...
    if (!in_array($extension, ['doc', 'pdf', 'docx', 'xls', 'xlsx','odt'])) {
        ...
        if (move_uploaded_file($file, $destination)) {
            $sql = "INSERT INTO rekini (user_name, mail ,name, size, downloads) VALUES ('$user_name','$e_mail','$filename', $size, 0)";
...
if (isset($_GET['file_id'])) {
    ...
    $sql = "SELECT * FROM rekini WHERE id=$id";
    $result = mysqli_query($conn, $sql);

    $file = mysqli_fetch_assoc($result);
    $filepath = 'rekini/' . $file['name'];
    ...
    if (file_exists($filepath)) {
        ...
        header('Content-Length: ' . filesize('rekini/' . $file['name']));
        readfile('rekini/' . $file['name']);

        // Now update downloads count
        $newCount = $file['downloads'] + 1;
        $updateQuery = "UPDATE rekini SET downloads=$newCount WHERE id=$id";
...
if (isset($_GET['del'])) {
    ...
    $sql2 = "DELETE FROM rekini WHERE id=$id";
    $result = mysqli_query($conn, $sql2);

    echo "<meta http-equiv='refresh' content='0;url=/cms/rekini.php'>";
    exit;
