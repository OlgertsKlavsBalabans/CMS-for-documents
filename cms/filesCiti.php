...
if (isset($_POST['save'])) { // if save button on the form is clicked
    ...
    $sql_e = "SELECT * FROM citi WHERE name='$filename'";
  	$res_e = mysqli_query($conn, $sql_e);
        
    
    // destination of the file on the server
    $destination = 'citi/' . $filename;
    ...
    if (!in_array($extension, ['doc', 'pdf', 'docx', 'xls', 'xlsx','odt'])) {
        ...
        if (move_uploaded_file($file, $destination)) {
            $sql = "INSERT INTO citi (user_name, mail ,name, size, downloads) VALUES ('$user_name','$e_mail','$filename', $size, 0)";
...
if (isset($_GET['file_id'])) {
    ...
    $sql = "SELECT * FROM citi WHERE id=$id";
    $result = mysqli_query($conn, $sql);

    $file = mysqli_fetch_assoc($result);
    $filepath = 'citi/' . $file['name'];
    ...
    if (file_exists($filepath)) {
        ...
        header('Content-Length: ' . filesize('citi/' . $file['name']));
        readfile('citi/' . $file['name']);

        // Now update downloads count
        $newCount = $file['downloads'] + 1;
        $updateQuery = "UPDATE citi SET downloads=$newCount WHERE id=$id";
...
if (isset($_GET['del'])) {
    ...
    $sql2 = "DELETE FROM citi WHERE id=$id";
    $result = mysqli_query($conn, $sql2);

    echo "<meta http-equiv='refresh' content='0;url=/cms/citi.php'>";
    exit;
